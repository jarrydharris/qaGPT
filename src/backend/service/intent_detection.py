...
from flask_session.sessions import RedisSession, ServerSideSession
...
def init_intent_executor(session: ServerSideSession) -> AgentExecutor:
    ...
    tools = [
        Tool(
            ...
            description=("Only use this tool if the user indicates they want to see animals or wildlife."
                         "provide one word to indicate the type of wildlife")
        ),
        Tool(
            func=destination.run,
            name="destination_textbox",
            description=("Only use this tool if the user indicates interest in seeing a destination."
    ...
    aim_callback = AimCallbackHandler(repo=".", experiment_name=f"Tool Agent Experiments V1 -> Datetime: {session}")
    callbacks = [StdOutCallbackHandler(), aim_callback]
    callback_manager = BaseCallbackManager(handlers=callbacks)
    agent = ZeroShotAgent(llm_chain=llm_chain, allowed_tools=tool_names, callbacks=callbacks)
    return AgentExecutor(agent=agent, tools=tools, verbose=True, callbackManager=callback_manager)

...
function buildImageUrl(base_url, size, imagePath) {
    return 'url("' + base_url + size + imagePath + '")';
}

function buildProductElement(title, release, language, genres, overview, backdrop_path, tmdb_id) {
    const product_element = document.createElement('div');
    product_element.classList.add('product');
    product_element.setAttribute('product_id', tmdb_id);
    product_element.style.backgroundImage = buildImageUrl(
        metadata["images"]["base_url"], metadata["images"]["backdrop_sizes"][1], backdrop_path
    );
    product_element.innerHTML = (
        '<div id="background-shader" class=' + tmdb_id + '>' +
        '<div id="title-text">' + title + '</div>' +
        '<div id="release-text">' + release + ' (' + language + ') ' + genres.join(', ') + '</div>' +
        '<div id="overview-text"><b>Overview </b> <br>' + overview + '</div>' +
        '</div>'
    )
...
export function updateProducts(product_ids) {
    const product_container = document.querySelectorAll('.product');
    if (product_ids.length === 0) {
        for (let i = 0; i < product_container.length; i++) {
            product_container[i].style.visibility = 'visible';
        }
    } else {
        for (let i = 0; i < product_container.length; i++) {
            const productId = parseInt(product_container[i].getAttribute('product_id'));
            if (product_ids.includes(productId)) {
                product_container[i].style.display = 'flex';
            } else {
...
export function displayProducts(products) {
    ...
    for (let i = 0; i < Object.keys(products.id).length; i++) {
        const product_element = buildProductElement(
            products["title"][i],
            products["release_date"][i],
            products["original_language"][i],
            products["genre_ids"][i],
            products["overview"][i],
            products["backdrop_path"][i],
            products["id"][i]
        );
        product_container.append(product_element.cloneNode(true));
    }
}
